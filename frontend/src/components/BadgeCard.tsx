...
export function BadgeCard() {
  ...
  const currentBadge = [...BADGES].reverse().find(b => completedTerms >= b.minTerms) || BADGES[0]
  const nextBadge = BADGES.find(b => b.minTerms > completedTerms)
  ...
  return (
    <div className={`relative surface-card rounded-[1.75rem] p-6 overflow-hidden`}>
      ...
      <div className="relative z-10 flex items-center justify-between gap-3 mb-5 flex-wrap">
        <div>
          <p className="text-xs text-slate-500 uppercase tracking-[0.16em] font-semibold mb-0.5 font-body">Saver Rewards</p>
          <p className="text-sm text-slate-300 font-body">Your live tier updates automatically after each completed term.</p>
        </div>
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between flex-wrap gap-6">
        ...
        <div className="flex-1 min-w-[200px] max-w-sm">
          <div className="flex justify-between text-xs text-slate-500 mb-2 font-mono-tabular">
            <span>{completedTerms} terms completed</span>
            {nextBadge && <span>Next: {nextBadge.name} at {nextBadge.minTerms}</span>}
          </div>
          <div className="h-2 bg-white/5 rounded-full overflow-hidden mb-3">
            <div
              className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-teal-400 transition-all duration-700"
              style={{ width: nextBadge ? `${Math.min(100, (completedTerms / nextBadge.minTerms) * 100)}%` : '100%' }}
            />
          </div>
          <div className="flex justify-between">
            {BADGES.map(b => (
              <div key={b.level} className="flex flex-col items-center gap-1.5 group/tier relative cursor-help">
                <div className={`w-2.5 h-2.5 rounded-full transition-colors duration-500 ${completedTerms >= b.minTerms ? 'bg-emerald-400' : 'bg-white/10'}`} />
                <div className="flex flex-col items-center text-center">
                  <span className="text-[10px] text-slate-400 font-medium font-body">{b.name.split(' ')[0]}</span>
                  <span className={`text-[9px] font-mono-tabular ${completedTerms >= b.minTerms ? 'text-[color:var(--color-accent-growth)]' : 'text-slate-600'}`}>{b.apy || 'Base'}</span>
                </div>
